...
import serviceAccountKey from "./react-js-blog-website-7e3be-firebase-adminsdk-3vhpf-fca16dd6f2.json" assert { type: "json" };
...
const generateUploadURL = async () => {
  ...
  return await s3.getSignedUrlPromise("putObject", {
    Bucket: "react-js-blogging-website-mark1",
    Key: imageName,
    Expires: 1000,
...
server.post("/signup", (req, res) => {
  ...
  // validating the data fromm frontend
  ...
  getAuth()
    ...
    .catch((err) => {
      return res.status(500).json({
        error:
          "Failed to authenticate you with google. Try with some other account",
      });
    });
});
...
server.post("/create-blog", verifyJWT, (req, res) => {
  ...
  if (!draft) {
    ...
    if (!tags.length || tags.length > 18) {
      return res
        .status(403)
        .json({ error: "Provide tags to publish the blog, Maximum 10" });
    }
  }
